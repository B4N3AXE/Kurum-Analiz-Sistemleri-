...
// Find the position of `{currentTab === 'mesaj' && 'MESAJ'}`
const textToFind = "{currentTab === 'mesaj' && 'MESAJ'}";
const idx = code.indexOf(textToFind);

if (idx !== -1) {
    const nextLineIdx = code.indexOf("{/* Tab: KAS.ai AI Chatbot */}", idx);
    if (nextLineIdx !== -1) {
        ...
        code = code.substring(0, idx) + replacement + code.substring(nextLineIdx);
